...
# data
CSV_HEADER  = "Instance,Result,Time, conflict_size, average_len, reduction, i_uip_reduction, mem_use\n"
Result      = namedtuple('Result', ('problem', 'result', 'elapsed', 'conflict_size', 'average_len', 'reduction', 'i_uip_reduction', 'mem_use' ))
...
SOLVERS = {
    ...
    "i-uip-mini-greedy"  : "./minisat_static -i-uip -i-mini -i-active -cpu-lim=2000"
        ...
        if (len(problem_set_name) > 0):
            ...
            if (exists):
                file_data = np.genfromtxt(filename, delimiter=',', dtype=None, encoding=None,
                              names=["Instance", "Result", "Time", "conflict_size", "average_len", "reduction", "i_uip_reduction",  "mem_use"],
                              skip_header=1)
                op = 'a+'
                ...
                if single_solver:
                    ...
                    for problem in problems:
                        if (problem.startswith(problem_set) ) and problem not in finished_instances:
                            ...
                            fp.write("%s,%s,%s, %s,%s, %s, %s\n" % (result.problem, result.result, result.elapsed, result.conflict_size, result.average_len, result.reduction, result.i_uip_reduction, result.mem_use))
